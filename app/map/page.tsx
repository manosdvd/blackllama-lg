...
import { campMapLocations } from "../../lib/camp-map";
...
export default function MapPage() {
  const campsiteCount = campMapLocations.filter((location) => location.category === "campsite").length;
  const programCount = campMapLocations.filter((location) => location.category === "program").length;
  ...
  return <main className="map-page">
    ...
    <section className="map-hero" aria-labelledby="map-page-title">
      ...
      <div className="map-hero-inner">
        ...
        <div className="map-hero-compass" aria-hidden="true">
          ...
          <span><strong>{campMapLocations.length}</strong> places to explore</span>
        </div>
      </div>
      <div className="map-hero-meta" aria-label="Map features">
        <div><span>01</span><strong>{campsiteCount} campsites</strong><small>See each site in camp context</small></div>
        <div><span>02</span><strong>{programCount} program areas</strong><small>Learn the camp layout</small></div>
    ...
    <section className="map-planning-note" aria-labelledby="map-planning-note-title">
      ...
      <p>Paths, assignments, program availability, fire conditions, and emergency procedures can change. Review <Link href="/guide/daily-camp-life">daily camp life</Link> and listen for the arrival-day safety briefing before moving through camp.</p>
    </section>
