...
export default function ProgramExplorer({ offerings }: { offerings: ProgramOffering[] }) {
  ...
  return <>
    ...
    <div className="program-groups">{grouped.map(([label, items]) => items.length > 0 && <section key={label}><header><h2>{label}</h2><span>{items.length} offering{items.length === 1 ? "" : "s"}</span></header><div>{items.map((item) => <article className="program-row" key={`${label}-${item.id}`}><div className="program-main"><span>{item.area} · {item.viability}</span><h3><Link href={`/merit-badges/${item.id}`}>{item.title}</Link>{item.eagleRequired && <i>Eagle</i>}</h3><p>{item.description}</p><Link className="badge-guide-link" href={`/merit-badges/${item.id}`}>About this badge &amp; official requirements →</Link><div className="program-meta"><span>{item.difficulty}</span><span>Capacity {item.capacity}</span><span>{item.fee ? `$${item.fee} materials` : "No fee listed"}</span></div></div><div className="program-slots">{item.slots.map((slot) => <Link key={slot.id} href={`/my-plan?add=${slot.id}`}><strong>{slot.label}</strong><small>{slot.location}</small><span>Add to plan →</span></Link>)}</div><details><summary>Camp planning details</summary><p><strong>Guidance:</strong> {item.ageGuidance}</p><p><strong>Prerequisites:</strong> {item.id === "emergency-preparedness" ? <><Link href="/merit-badges/first-aid">First Aid merit badge</Link> is required.</> : item.prerequisites}</p><p><strong>Outside work:</strong> {item.outsideWork}</p><p className="camp-guidance-note">These are Camp Lawton planning notes. The official badge page controls advancement requirements.</p></details></article>)}</div></section>)}</div>
  </>;
}
