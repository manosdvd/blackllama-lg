      ...
      <div className="interest-table">
        ...
        {visible.map((badge) => {
          ...
          return <div key={badge.id} className={`interest-row ${interest.count > 0 ? "selected" : ""}`}>
            <div className="interest-badge"><span>{badge.area} · Tier {badge.tier}</span><strong><Link href={`/merit-badges/${badge.id}`} target="_blank" rel="noreferrer">{badge.title}<span className="sr-only"> field guide (opens in a new tab)</span></Link></strong><small className={`completion-${badge.completion}`}>{completionLabels[badge.completion]}</small></div>
            ...
            <details className="survey-badge-details"><summary>Camp workload &amp; badge guide</summary><dl><div><dt>Prerequisites</dt><dd>{badge.id === "emergency-preparedness" ? <><Link href="/merit-badges/first-aid" target="_blank" rel="noreferrer">First Aid merit badge<span className="sr-only"> field guide (opens in a new tab)</span></Link> is required.</> : badge.prerequisites ?? "None listed"}</dd></div><div><dt>Outside camp</dt><dd>{badge.unavailableAtCamp ?? "No unavailable requirement listed"}</dd></div><div><dt>Estimated class time</dt><dd>{badge.classHours} hour{badge.classHours === 1 ? "" : "s"}</dd></div><div><dt>Estimated individual time</dt><dd>{badge.individualEffort.raw === "N/A" ? "Not available" : `${badge.individualEffort.raw} hour${badge.individualEffort.minimumHours === 1 ? "" : "s"}`}</dd></div></dl><div className="survey-badge-resource"><p>Camp estimates support planning; current official requirements and counselor review govern completion.</p><Link href={`/merit-badges/${badge.id}`} target="_blank" rel="noreferrer">Open the {badge.title} field guide <span aria-hidden="true">↗</span><span className="sr-only"> (opens in a new tab)</span></Link></div></details>
