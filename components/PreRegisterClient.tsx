...
import MeritBadgeSurveyStep, { type SurveyInterestState } from "./MeritBadgeSurveyStep";
...
function savedString(source: Record<string, unknown>, key: string, fallback = ""): string { return typeof source[key] === "string" ? String(source[key]).slice(0, 200) : fallback; }
function savedCount(source: Record<string, unknown>, key: string): number { const value = Number(source[key]); return Number.isFinite(value) ? Math.max(0, Math.min(250, Math.floor(value))) : 0; }

function normalizeDraft(value: unknown): FormState {
  const source = record(value), rawInterests = record(source.interests);
  const interests = Object.fromEntries(meritBadgeSurveyCatalog.flatMap((badge) => {
    const item = record(rawInterests[badge.id]), count = savedCount(item, "count");
    if (!count && !item.note && !item.priority) return [];
    const priority = savedString(item, "priority", "nice-to-have");
    return [[badge.id, { count, priority: validPriorities.has(priority) ? priority : "nice-to-have", note: savedString(item, "note").slice(0, 250) }]];
  }));
  return {
...
export default function PreRegisterClient() {
  ...
  const interestRows = useMemo(() => meritBadgeSurveyCatalog.filter((badge) => (form.interests[badge.id]?.count ?? 0) > 0), [form.interests]);
  const update = (name: keyof FormState, value: FormState[keyof FormState]) => setForm((current) => ({ ...current, [name]: value }));
  const updateCount = (name: "youthMale" | "youthFemale" | "youthOther" | "adultMale" | "adultFemale" | "adultOther", value: string) => setForm((current) => {
    const next = { ...current, [name]: Math.max(0, Math.min(250, Math.floor(Number(value) || 0))) };
    if (!name.startsWith("youth")) return next;
