...
export default function GuideDirectory({ articles }: { articles: GuideArticle[] }) {
  ...
  return <>
    ...
    <div className="guide-directory">
      {categories.map((category) => <section key={category} className="guide-category"><h2>{category}</h2><div>{visible.filter((article) => article.category === category).map((article) => <Link href={`/guide/${article.slug}`} key={article.slug} className={`directory-article priority-${article.priority}`}><span>{article.audience === "all" ? "All sessions" : article.audience === "bsa" ? "BSA weeks" : article.audience === "cub" ? "Cub weekend" : "Leaders"}</span><h3>{article.title}</h3><p>{article.summary}</p><small>Reviewed {article.updatedAt ? new Date(article.updatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }) : "Review pending"}</small></Link>)}</div></section>)}
