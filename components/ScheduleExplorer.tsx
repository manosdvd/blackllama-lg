...
export default function ScheduleExplorer({ bsaEvents, cubEvents }: { bsaEvents: PublicScheduleEvent[]; cubEvents: PublicScheduleEvent[] }) {
  ...
  return <>
    ...
    <div className="day-tabs schedule-day-tabs" role="tablist" aria-label="Choose schedule day">{days.map((day) => <button key={day} role="tab" aria-selected={effectiveDay === day} onClick={() => setSelectedDay(day)}>{day}</button>)}</div>
